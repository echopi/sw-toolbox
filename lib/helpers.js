  ...
  return fetch(request.clone()).then(function(response) {
    ...
    if (request.method === 'GET' && successResponses.test(response.status)) {
      openCache(options).then(function(cache) {
        var request_ = rewrite(request,options);

        cache.put(request_, response).then(function() {
          ...
          if ((cacheOptions.maxEntries || cacheOptions.maxAgeSeconds) &&
              cacheOptions.name) {
            queueCacheExpiration(request_, cache, cacheOptions);
          }
        });
      });
    }
...
function rewrite(request, options){
  if(options && request){
    if(typeof options.rewrite === "string"){
      return new Request(options.rewrite, {credentials: request.credentials, headers: request.headers, method: request.method});
    }

    if(typeof options.rewrite === "function"){
      var rewriteUrl = options.rewrite(request.url);
      if(rewriteUrl){
        return new Request(rewriteUrl, {credentials: request.credentials, headers: request.headers, method: request.method});
      }
    }
  }
  return request;
